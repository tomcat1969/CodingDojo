...
select countries.name as countryName,count(cities.name)
from countries
...
--with only Constitutional Monarchy with a capital greater than 200 and a life expectancy greater than 75 years? (1)

select countries.name, countries.government_form,countries.indep_year,countries.capital
from countries
where countries.government_form like "Constitutional Monarchy" 
and countries.capital > 200
and year(NOW()) - countries.indep_year > 75
...
select countries.region, count(countries.id) as "the number of countries"
from countries
